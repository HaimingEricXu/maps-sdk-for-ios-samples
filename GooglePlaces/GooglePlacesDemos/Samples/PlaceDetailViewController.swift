...
//  GooglePlacesDemos
//
//  Created by Chris Arriola on 5/28/20.
//

import GooglePlaces
import UIKit

struct TestBoi : OptionSet {
    var rawValue: UInt
        
    public static var test1: TestBoi = TestBoi(rawValue: 1)
    
    public static var test2: TestBoi = TestBoi(rawValue: 2)
...
@objc
class PlaceDetailViewController : BaseDemoViewController {
    
    let placesClient = GMSPlacesClient.shared()
    
    override class func demoTitle() -> String! {
        return "Place Detail (Swift)"
    }
    ...
    override func viewDidLoad() {
        ...
        let fields: GMSPlaceField = [GMSPlaceField.name, GMSPlaceField.phoneNumber]
        
        placesClient.fetchPlace(fromPlaceID: "ChIJz8RiHtqGhYARDijrpUMCFlI", placeFields: fields, sessionToken: nil) { place, error in
            ...
            textView.text = "Got place \(place)"
        }
    }
}

extension GMSPlaceField {
    static func fields(_ fields: GMSPlaceField...) -> GMSPlaceField {
        return GMSPlaceField(rawValue:
            fields.map { $0.rawValue }.reduce(0, { $0 | $1})
        )!
    }
}
